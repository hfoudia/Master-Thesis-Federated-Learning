...
# FOR TESTING THE CIC IDS 2017 DATASET, SCOPE OF MY MASTER THESIS
IDS2017_FLOWS = {'attackers': ['172.16.0.1', '172.16.0.11', '192.168.10.8', '205.174.165.73', '205.174.165.69', '205.174.165.70', '205.174.165.71'],
                 'victims': ['192.168.10.50', '205.174.165.68', '192.168.10.51', '205.174.165.66', '192.168.10.19', '192.168.10.17', '192.168.10.16',
                             '192.168.10.12', '192.168.10.9', '192.168.10.5', '192.168.10.8', '192.168.10.14', '192.168.10.15', '192.168.10.25']}
...
def parse_labels(dataset_type=None, attackers=None,victims=None, label=1):
    ...
    elif dataset_type is not None and dataset_type in ATTACK_SPECS:
        DDOS_FLOWS = ATTACK_SPECS[dataset_type]
    else:
        return None
...
def main(argv):
    ...
    if args.dataset_folder is not None and args.dataset_type is not None:
        ...
        log_string = time.strftime("%Y-%m-%d %H:%M:%S") + " | dataset_type:" + args.dataset_type[0] + \
                     " | flows (tot,ben,malicious):(" + str(total_flows) + "," + str(benign_flows) + "," + str(ddos_flows) + \
                     ") | fragments (tot,ben,malicious):(" + str(total_fragments) + "," + str(benign_fragments) + "," + str(ddos_fragments) + \
    ...
    if args.preprocess_folder is not None or args.preprocess_file is not None:
        ...
        if args.no_split == True: # don't split the dataset
            ...
            log_string = time.strftime("%Y-%m-%d %H:%M:%S") + " | Total examples (tot,ben,malicious):(" + str(total_examples) + "," + str(total_benign_examples) + "," + str(total_ddos_examples) + \
            ...
            log_string = time.strftime("%Y-%m-%d %H:%M:%S") + " | examples (tot,ben,malicious):(" + str(total_examples) + "," + str(total_benign_examples) + "," + str(total_ddos_examples) + \
    ...
    if args.balance_folder is not None and args.output_folder is not None:
        ...
        log_string = time.strftime("%Y-%m-%d %H:%M:%S") + " | total_flows (tot,ben,malicious):(" + str(total_flows) + "," + str(benign_flows) + "," + str(ddos_flows) + \
                     ") | Packets (train,val,test):(" + str(train_packets) + "," + str(val_packets) + "," + str(test_packets) + \
